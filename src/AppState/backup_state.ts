...
export const AppStateMachine = createMachine<FlowContext>(
  ...
  },
  {
    actions: {
      ...
      hadleActionSort: itemReposition,
    },
  }
);
...
function actionFromRecording(context: FlowContext, event: any) {
  ...
  newRecordedAction["svg"] = InteractionDefintions.filter(
    (idata) =>
      idata.name.toLowerCase() === newRecordedAction.actionType.toLowerCase()
  )[0].svg;
...
function itemReposition(context: FlowContext, event: any){
  const { dragIndex, index } = event.dragInfo;
  if (dragIndex !== index) {
    const newItems = [...context.flowActions];
    const dragItem = newItems[dragIndex];
    newItems.splice(dragIndex, 1);
    newItems.splice(index, 0, dragItem);
    assign({ flowActions: newItems });
  }
}